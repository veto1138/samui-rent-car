            ...
            <div class="bg-white shadow-sm rounded-lg overflow-hidden mb-6">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900">แก้ไขข้อมูลรถยนต์</h2>
                            <p class="text-gray-600 mt-1">แก้ไขข้อมูลรถยนต์: {{ $car->full_name }}</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="col-span-2">
                                ...
                                <input type="text" name="full_name" id="full_name"
                                    value="{{ old('full_name', $car->full_name) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="brand_name" id="brand_name"
                                    value="{{ old('brand_name', $car->brand_name) }}"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            ...
                            <div>
                                ...
                                <select name="status" id="status"
                                    ...
                                    <option value="available"
                                        {{ old('status', $car->status) == 'available' ? 'selected' : '' }}>พร้อมใช้งาน
                                    </option>
                                    <option value="rented" {{ old('status', $car->status) == 'rented' ? 'selected' : '' }}>
                                        ถูกเช่า</option>
                                    <option value="maintenance"
                                        {{ old('status', $car->status) == 'maintenance' ? 'selected' : '' }}>ซ่อมบำรุง
                                    </option>
                                </select>
                                @error('status')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            ...
                            <div class="col-span-2">
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50 overflow-hidden"
                                        id="imagePreview">
                                        @if ($car->image)
                                            <img src="{{ asset('storage/' . $car->image) }}" alt="รูปรถปัจจุบัน"
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @if ($car->image)
                                            <p class="text-sm text-blue-600 mt-1">รูปภาพปัจจุบัน:
                                                {{ basename($car->image) }}</p>
                                        @endif
                                    </div>
                                </div>
                                @error('image')
    ...
    <script>
        function previewImage(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                // กลับไปแสดงรูปภาพเดิม
                @if ($car->image)
                    preview.innerHTML = `
                        <img src="{{ asset('storage/' . $car->image) }}" alt="รูปรถปัจจุบัน" class="w-full h-full object-cover">
                    `;
                @else
                    preview.innerHTML = `
                        <div class="text-center">
